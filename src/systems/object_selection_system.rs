...
fn get_root_entities(state: &State) -> Vec<Entity> {
...
pub fn object_selection_system(state: &mut State, context: &Context) {
    ...
    context.input(|input| {
        ...
        let world_position = camera.window_space_to_world_space(screen_position, screen_rect);
        let max_distance_to_select = camera.get_max_distance_to_select();
        let Some(selected_object) = get_selected_object(state, world_position, max_distance_to_select) else {
            return;
        };
        if state.selected == selected_object {
            return;
        }
        state.set_selected(selected_object);
    });
}