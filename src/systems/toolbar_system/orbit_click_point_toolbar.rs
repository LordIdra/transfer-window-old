...
use eframe::{egui::{Context, Window, Image, ImageButton, Ui, Layout, Label}, emath::{Align2, Align}, epaint::{self, Color32}};
...
fn warp_to_point(state: &mut State, orbit_click_point: OrbitClickPoint) {
    state.current_warp = Some(WarpDescription { 
        start_time: state.time, 
        end_time: orbit_click_point.get_time() 
    });
}
